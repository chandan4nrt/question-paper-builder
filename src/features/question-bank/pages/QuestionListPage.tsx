...
import type { Difficulty, QuestionFilters, QuestionLanguage, QuestionStatus, QuestionType } from '../types/question';
...
const LANGUAGES: QuestionLanguage[] = ['HINDI', 'ENGLISH', 'BILINGUAL'];
  ...
  return (
    <div className="page">
      ...
      <div className="button-row" style={{ margin: '1rem 0' }}>
        ...
        />
        <select onChange={(e) => updateFilter('questionType', e.target.value as QuestionType)}>
          <option value="">All Types</option>
          {QUESTION_TYPES.map((t) => (
            <option key={t} value={t}>{t}</option>
          ))}
        </select>
        <select onChange={(e) => updateFilter('difficulty', e.target.value as Difficulty)}>
          <option value="">All Difficulties</option>
          {DIFFICULTIES.map((d) => (
            <option key={d} value={d}>{d}</option>
          ))}
        </select>
        <select onChange={(e) => updateFilter('language', e.target.value as QuestionLanguage)}>
          <option value="">All Languages</option>
          {LANGUAGES.map((l) => (
            <option key={l} value={l}>{l}</option>
          ))}
        </select>
        <select onChange={(e) => updateFilter('status', e.target.value as QuestionStatus)}>
