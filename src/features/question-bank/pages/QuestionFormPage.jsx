...
const OPTION_BASED_TYPES = ['MCQ', 'MULTIPLE_SELECT', 'TRUE_FALSE'];
const ANSWER_BASED_TYPES = ['FILL_IN_THE_BLANK', 'ONE_WORD', 'SHORT_ANSWER', 'LONG_ANSWER'];

export function QuestionFormPage({ mode }) {
  const { id } = useParams();
          ...
          <fieldset style={{ marginTop: '1rem', border: '1px solid var(--border)', borderRadius: 'var(--radius-sm)', padding: '1rem' }}>
            ...
            {fields.map((field, index) => (
              <div key={field.id} style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem', alignItems: 'center' }}>
                ...
                />
                <label>
                  <input type="checkbox" {...register(`options.${index}.isCorrect`)} /> Correct
                </label>
            ...
            {formState.errors.options && (
              <span className="error-text">{formState.errors.options.message}</span>
            )}
          </fieldset>
        )}
