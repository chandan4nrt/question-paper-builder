import type { PaperState, LabelMarker } from './types';
...
interface MetadataQuestion {
  ...
  subQuestions?: MetadataSubQuestion[];
...
function createUniqueName(used: Set<string>, base: string | null | undefined, fallback: string): string {
...
function nextImageKey(counter: { current: number }): string {
  counter.current += 1;
  return `IMG_${String(counter.current).padStart(3, '0')}`;
}

export function buildSetPaperFormData(state: PaperState): { formData: FormData; payload: MetadataPayload } {
  const images: StoredImage[] = [];
  const usedNames = new Set<string>();
  const mappedQuestions: MetadataQuestion[] = [];
  ...
  for (const q of state.questions) {
    switch (q.type) {
      ...
      case 'label': {
        ...
        let labelPicture: string | undefined;
        if (q.image) {
          const imgKey = nextImageKey(imageKeyCounter);
