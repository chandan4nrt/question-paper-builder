...
interface AddQuestionModalProps {
  ...
  onAdd: (question: QuestionResponse, marks: number, sectionName: string) => void;
  onClose: () => void;
  adding: boolean;
  error?: string;
}
...
export function AddQuestionModal({
  ...
  const { data, isLoading } = useQuestionList({
    ...
    status: 'APPROVED',
    questionType: (questionType || undefined) as never,
    page,
    limit,
  });
        ...
        {selected && (
          <div className="card" style={{ marginTop: '1rem', background: '#fafafa' }}>
            ...
            <div className="button-row" style={{ marginTop: '0.75rem', alignItems: 'flex-end' }}>
              ...
              <button
                onClick={() => {
                  const marksInput = document.getElementById('add-question-marks') as HTMLInputElement;
                  const sectionInput = document.getElementById('add-question-section') as HTMLInputElement;
                  onAdd(
                    selected,
